...
;
; The outputs for this program are a file 'vcirc_model_free_plateifu.txt'
; that shows the circular velocity of the best fit model, and
; a file 'kinemetry_params_free_plateifu.txt', which holds the kinematic best
; fitting PA and the asymmetry parameters.
;
; For this code, kinemetry allows PA and q to vary.
;
; Becky Nevin
;###############################################################

PRO run_kinemetry_on_multiple_galaxies_free_params, plateid

string_underlined = STRJOIN(STRSPLIT(STRING(plateid),'-', /EXTRACT), '_')

file = 'kinemetry_input_'+STRING(plateid)+'.txt'
...
KINEMETRY, xbin, ybin, velbin, rad, pa, q, cf, X0=xcen,Y0=ycen, ntrm=10, ERROR=er_velbin, name='simulation',RANGEPA=[MEDIAN(pa)-90-1,MEDIAN(pa)-90+1],er_cf=er_cf, er_pa=er_pa, er_q=er_q,/ALL, /FIXCEN, /plot

PAQ1 = [PA_before, MEDIAN(q)]
PAQ = Reform(Rebin(PAQ1, 2, 100), 200)
PRINT, 'about to do only one term'

KINEMETRY, xbin, ybin, velbin, rad, pa, q, cf, X0=xcen,Y0=ycen, ntrm=10, ERROR=er_velbin,er_cf=er_cf, er_pa=er_pa, er_q=er_q,velcirc=velCirc,  velkin=velKin, /ALL, /BMODEL, /FIXCEN, /plot
...
KINEMETRY, xbin, ybin, sigbin, rad, pa, q, cf, X0=xcen, Y0=ycen,ntrm=10, scale=0.5, ERROR=er_sigbin, name='sigma map', er_cf=er_cf, er_pa=er_pa, er_q=er_q, /verbose, /ALL, /plot, /FIXCEN
k0 = cf[*,0]
k1 = SQRT(cf[*,1]^2 + cf[*,2]^2)
k2 = SQRT(cf[*,3]^2 + cf[*,4]^2)
k3 = SQRT(cf[*,5]^2 + cf[*,6]^2)
