;################################################################
; This program delivers stellar velocities, stellar velocity dispersion, and errors
; to kinemetry and fits them first with a varying PA,
; then, holding the PA fixed, fit them again and determine
; both asymmetry parameters (v and sig).
;
; The outputs for this program are a file 'vcirc_model_plateifu.txt'
; that shows the circular velocity of the best fit model, and
; a file kinemetry_params_plateifu.txt, which holds the kinematic best
; fitting PA and the asymmetry parameters.
;
; The center of the kinematic map is determined using the Radon Transform first
; (see Nevin et al. 2020)
; Also of note, the data is Voronoi binned, so this is presented to the code
; by assigning each spaxel in a bin the same value for stellar velocity, disp, etc
;
; To run in IDL, make sure you have the IDL astronomers library
; idl
; .r run_kinemetry_XXXX-XXXXX
; run_kinemetry_XXXX-XXXXX
;
; Becky Nevin
;###############################################################

PRO run_kinemetry_8318-12704
