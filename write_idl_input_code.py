def write_kinemetry_idl(plate, ifu):
	...
	file=open('run_kinemetry_'+str(plate)+'-'+str(ifu)+'.pro','w')
	file.write(';################################################################'+'\n')
	file.write('; This program delivers stellar velocities, stellar velocity dispersion, and errors'+'\n')
	file.write('; to kinemetry and fits them first with a varying PA,'+'\n')
	file.write('; then, holding the PA fixed, fit them again and determine'+'\n')
	file.write('; both asymmetry parameters (v and sig).'+'\n')
	file.write(';'+'\n')
	file.write("; The outputs for this program are a file 'vcirc_model_plateifu.txt'"+'\n')
	file.write('; that shows the circular velocity of the best fit model, and'+'\n')
	file.write('; a file kinemetry_params_plateifu.txt, which holds the kinematic best'+'\n')
	file.write('; fitting PA and the asymmetry parameters.'+'\n')
	file.write(';'+'\n')
	file.write('; The center of the kinematic map is determined using the Radon Transform first'+'\n')
	file.write('; (see Nevin et al. 2020)'+'\n')
	file.write('; Also of note, the data is Voronoi binned, so this is presented to the code'+'\n')
	file.write('; by assigning each spaxel in a bin the same value for stellar velocity, disp, etc'+'\n')
	file.write(';'+'\n')
	file.write('; To run in IDL, make sure you have the IDL astronomers library'+'\n')
	file.write('; idl'+'\n')
	file.write('; .r run_kinemetry_XXXX-XXXXX'+'\n')
	file.write('; run_kinemetry_XXXX-XXXXX')
	file.write(';'+'\n')
	file.write('; Becky Nevin'+'\n')
	file.write(';###############################################################'+'\n')
	file.write('\n')
	file.write('PRO run_kinemetry_'+str(plate)+'-'+str(ifu)+'\n')
	file.write('\n')
	...
	file.write("file_out='vcirc_model_"+str(plate)+"-"+str(ifu)+".txt'"+'\n')
	...
	file.write("file_out_2 = 'kinemetry_params_"+str(plate)+"-"+str(ifu)+".txt'"+'\n')
